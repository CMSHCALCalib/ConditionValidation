...
void compare( ) 
{
	//char* file0 = "HCAL/producers/NOV2017/HEHF/HcalGains_2017NovReReco_IOV18.txt"; //new
	//char* file1 = "HCAL/producers/2017SeptReReco/HcalGains_HEP17PhiSym.txt"; //previous

	const char* file0 = "HcalGains_2017NovReReco_IOV18.txt"; //new
	const char* file1 = "HcalGains_HEP17PhiSym.txt"; //previous

    //here you can change z-ranges for different
	float zr = 0.25; // z-range amplitude
	float r_a = 1 - 1 * zr, r_b = 1 + 1 * zr;//hb
	float r3_a = 1 - 1 * zr, r3_b = 1 + 1 * zr;//he
	float r4_a = 1 - 1 * zr, r4_b = 1 + 1 * zr;//ho
	float r2_a = 1 - 0.25 * zr, r2_b = 1 + 0.25 * zr;//hf

	float outR = 0.30;  // z-range amplitude for oputliers
  ...
  int ieta2,iphi2,depth2, ieta1,iphi1,depth1;
  float c1[4], c2[4];
  ...
  for (int j=0; j<99999; j++) {
	...
	fscanf(ft0, "%d%d%d%s%f%f%f%f%s\r", &ieta1, &iphi1, &depth1, SubDet1, &c1[0], &c1[1], &c1[2], &c1[3], DetID1);

	float gain1 = c1[0] + c1[1] + c1[2] + c1[3];

	  if (SubDet1[1] == '\B') {
		  if (depth1 == 1) hHB_d1->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 2) hHB_d2->Fill(ieta1, iphi1, gain1);
	  }
	  if (SubDet1[1] == '\O') {
		  if (depth1>0)hHO_d1->Fill(ieta1, iphi1, gain1);
	  }
	 
	  if (SubDet1[1] == '\E') {
		  if (depth1 == 1)hHE_d1->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 2)hHE_d2->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 3)hHE_d3->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 4)hHE_d4->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 5)hHE_d5->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 6)hHE_d6->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 7)hHE_d7->Fill(ieta1, iphi1, gain1);
	  }
	  if (SubDet1[1] == '\F') {
		  if (depth1 == 1)hHF_d1->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 2)hHF_d2->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 3)hHF_d3->Fill(ieta1, iphi1, gain1);
		  if (depth1 == 4)hHF_d4->Fill(ieta1, iphi1, gain1);
	  }
  }
  ...
  for (int k = 0; k<99999; k++) {
	  ...
	  fscanf(ft1, "%d%d%d%s%f%f%f%f%s\r", &ieta2, &iphi2, &depth2, SubDet2, &c2[0], &c2[1], &c2[2], &c2[3], DetID2);
	  
	  float gain2 = c2[0] + c2[1] + c2[2] + c2[3];

	  if (SubDet2[1] == 'B') {
		  if (depth2 == 1) hHB_d1b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 2) hHB_d2b->Fill(ieta2, iphi2, gain2);
	  }
	  if (SubDet2[1] == 'O') {
		  if (depth2>0)hHO_d1b->Fill(ieta2, iphi2, gain2);
	  }

	  if (SubDet2[1] == 'E') {
		  if (depth2 == 1)hHE_d1b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 2)hHE_d2b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 3)hHE_d3b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 4)hHE_d4b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 5)hHE_d5b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 6)hHE_d6b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 7)hHE_d7b->Fill(ieta2, iphi2, gain2);
	  }
	  if (SubDet2[1] == 'F') {
		  if (depth2 == 1)hHF_d1b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 2)hHF_d2b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 3)hHF_d3b->Fill(ieta2, iphi2, gain2);
		  if (depth2 == 4)hHF_d4b->Fill(ieta2, iphi2, gain2);
	  }
