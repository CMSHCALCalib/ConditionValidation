...
void compare() {
	...
	const char* file0 = "HcalPedestals_ADC_2017plan1_v4.0_data_IOV2.txt"; //new
	const char* file1 = "HcalPedestals_ADC_2017plan1_v4.0_data_IOV1.txt"; //prev
  ...
  for (int j=0; j<99999; j++) {
    if (feof(ft0)!=0) break;
      fscanf(ft0,"%d%d%d%s%f%f%f%f",&ieta0,&iphi0,&depth0,SubDet1, &cov[0], &cov[1], &cov[2], &cov[3]);
                                                fscanf(ft0, "%f%f%f%f%s\n", &cov[4], &cov[5], &cov[6], &cov[7], DetID1);
   
      float av_peda = (cov[0]+cov[1]+cov[2]+cov[3])/4 + 999;
  ...
  for (int k = 0; k<99999; k++) {
	  ...
	  fscanf(ft1, "%d%d%d%s%f%f%f%f", &ieta2, &iphi2, &depth2, SubDet2, &covb[0], &covb[1], &covb[2], &covb[3]);
	  fscanf(ft1, "%f%f%f%f%s\n", &covb[4], &covb[5], &covb[6], &covb[7], DetID2);

	  float av_pedb = (covb[0] + covb[1] + covb[2] + covb[3]) / 4;
	  if (SubDet2[1] == 'B') {
  ...
  for (int i = 1; i <= 83; i++) {
	  for (int k = 1; k <= 72; k++) {
		  if (fabs(hHB_d1->GetBinContent(i, k)) > outR && hHB_d1->GetBinContent(i, k) != -999) cout << i  -42 << "    " << k << "     " << 1 << "     HB     " << hHB_d1->GetBinContent(i, k) << endl;
		  if (fabs(hHB_d2->GetBinContent(i, k)) > outR && hHB_d2->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 1 << "     HB     " << hHB_d2->GetBinContent(i, k) << endl;	 
	  }
  }
  for (int i = 1; i <= 83; i++) {
	  for (int k = 1; k <= 72; k++) {
		  if (fabs(hHO_d1->GetBinContent(i, k)) > outR && hHO_d1->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 4 << "     HO     " << hHO_d1->GetBinContent(i, k) << endl;		  
	  }
  }
  for (int i = 1; i <= 83; i++) {
	  for (int k = 1; k <= 72; k++) {
		  if (fabs(hHE_d1->GetBinContent(i, k)) > outR && hHE_d1->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 1 << "     HE     " << hHE_d1->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d2->GetBinContent(i, k)) > outR && hHE_d2->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 2 << "     HE     " << hHE_d2->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d3->GetBinContent(i, k)) > outR && hHE_d3->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 3 << "     HE     " << hHE_d3->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d4->GetBinContent(i, k)) > outR && hHE_d4->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 4 << "     HE     " << hHE_d4->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d5->GetBinContent(i, k)) > outR && hHE_d5->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 5 << "     HE     " << hHE_d5->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d6->GetBinContent(i, k)) > outR && hHE_d6->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 6 << "     HE     " << hHE_d6->GetBinContent(i, k) << endl;
		  if (fabs(hHE_d7->GetBinContent(i, k)) > outR && hHE_d7->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 7 << "     HE     " << hHE_d7->GetBinContent(i, k) << endl;
	  }
  }
  for (int i = 1; i <= 83; i++) {
	  for (int k = 1; k <= 72; k++) {
		  if (fabs(hHF_d1->GetBinContent(i, k)) > outR && hHF_d1->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 1 << "     HF     " << hHF_d1->GetBinContent(i, k) << endl;
		  if (fabs(hHF_d2->GetBinContent(i, k)) > outR && hHF_d2->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 2 << "     HF     " << hHF_d2->GetBinContent(i, k) << endl;
		  if (fabs(hHF_d3->GetBinContent(i, k)) > outR && hHF_d3->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 3 << "     HF     " << hHF_d3->GetBinContent(i, k) << endl;
		  if (fabs(hHF_d4->GetBinContent(i, k)) > outR && hHF_d4->GetBinContent(i, k) != -999) cout << i - 42 << "    " << k << "     " << 4 << "     HF     " << hHF_d4->GetBinContent(i, k) << endl;
	  }
  }
